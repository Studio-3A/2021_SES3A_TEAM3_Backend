...
import { DirectionsResponseStatus } from "./directions";
import { BadRequest, ErrorResponse, isErrorResponse, StatusCode } from "../common/expresstypes";

const nearbyPlacesSearchUrl = "https://maps.googleapis.com/maps/api/place/nearbysearch/json?";
...
export const getPlaces = async (params: string) => {
    ...
    if (!isErrorResponse(resp) && resp.status !== "OK") return BadRequest(resp.status, resp);
    return resp;
};

export const getPlacesByLocation = (details: NearbyPlacesInput) => {
    let qs = `key=${keys.placesbit}&location=${encodeURIComponent(details.lat + "," + details.lng)}&radius=${details.radius}`;
...
export interface NearbyPlacesInput extends Coordinate {
    ...
    type?: QueryPlaceType, //i.e. restaurant
};
...
type QueryPlaceType = 'accounting' | 'airport' | 'amusement_park' | 'aquarium' | 'art_gallery' | 'atm' | 'bakery' | 'bank' | 'bar' | 'beauty_salon' | 'bicycle_store' | 'book_store' | 'bowling_alley'
    ...
    | 'light_rail_station' | 'liquor_store' | 'local_government_office' | 'locksmith' | 'lodging' | 'meal_delivery' | 'meal_takeaway' | 'mosque' | 'movie_rental' | 'movie_theater' | 'moving_company'
    | 'museum' | 'night_club' | 'painter' | 'park' | 'parking' | 'pet_store' | 'pharmacy' | 'physiotherapist' | 'plumber' | 'police' | 'post_office' | 'primary_school' | 'real_estate_agency'
    | 'restaurant' | 'roofing_contractor' | 'rv_park' | 'school' | 'secondary_school' | 'shoe_store' | 'shopping_mall' | 'spa' | 'stadium' | 'storage' | 'store' | 'subway_station' | 'supermarket'
